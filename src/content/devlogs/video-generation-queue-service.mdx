...
`blpop` blocks until a job arrives — no polling, no sleep loop. Status transitions are written back to Redis at each stage so the client always has a current view: `queued` → `processing` → `completed`.
